...
const Community: React.FC = () => {
  ...
  const fetchFollowingPosts = async () => {
    try {
      const res = await fetch(
        `https://api.reparv.in/territoryapp/user/${auth?.user?.id}/following-posts`,
      );
      const data = await res.json();
      setPost(data);
      console.log(data, 'ff');
    ...
    useCallback(() => {
      if (selectedTab === 'discussions') {
        const fetchFollowersAndFollowing = () => {
          fetch(
            `https://api.reparv.in/territoryapp/user/${auth?.user?.id}/followers`,
          )
            .then(res => res.json())
            .then(setFollowers);
      ...
      <View style={styles.frame135}>
        ...
        >
          ...
          >
            <BookA
              size={21}
              color={selectedTab === 'MyPost' ? '#0078DB' : 'gray'}
              strokeWidth={2}
            />
          </View>
          <Text
