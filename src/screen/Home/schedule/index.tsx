...
const getStatusStyleText = (status: string) => {
  switch (status) {
    case 'New':
      return { color: '#22C55E', backgroundColor: '#DCFCE7' }; // green
    case 'Visit Scheduled':
...
const StatusCard: React.FC<Props> = ({ enquiry }) => {
  ...
  const expiryKey = `autoRejectExpiry-${enquiry.enquirersid}`;
  ...
  const startAutoRejectTimer = async () => {
    const storedExpiry = await AsyncStorage.getItem(expiryKey);
  ...
  const autoRejectIfExpired = async () => {
    const storedExpiry = await AsyncStorage.getItem(expiryKey);

    if (storedExpiry && Date.now() >= parseInt(storedExpiry)) {
      console.log('🚫 Auto-reject triggered!');
      await rejectEnquiry();
    } else {
      console.log('✅ Still within time window.');
    }
  };

  const clearAutoReject = async () => {
    if (timeoutRef.current) clearTimeout(timeoutRef.current);
    await AsyncStorage.removeItem(expiryKey);
  };

  const acceptEnquiry = async () => {
    await clearAutoReject();
    try {
      const response = await fetch(
      ...
      if (!response.ok) throw new Error('Failed to accept enquiry.');

      Toast.show({ type: 'success', text1: 'Enquiry Accepted' });
    } catch (error) {
  ...
  const rejectEnquiry = async () => {
    ...
    try {
      ...
      if (!response.ok) throw new Error('Failed to reject enquiry.');

      Toast.show({ type: 'error', text1: 'Enquiry Rejected' });
    } catch (error) {
  ...
  return (
    <View style={Statusstyles.card}>
      ...
      <Text style={Statusstyles.infoText}>
        {data?.fullname} is assigned to {property?.propertyName} property.
      </Text>
      ...
      <View style={Statusstyles.container}>
        ...
        <InfoRow
          ...
          value={`₹${formatIndianAmount(enquiry?.commissionAmount * 0.2)}`}
          icon={<HandCoins strokeWidth={0.5} color="gray" />}
        />
      </View>
